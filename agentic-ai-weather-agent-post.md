...
```
User Input → AI Planning → Points API → Forecast API → AI Processing → Response
```

1. **User Input** — Accepts city names, ZIP codes, coordinates, or even natural descriptions like *"national park near Homestead in Florida"*
2. **AI Planning** — Claude determines the correct coordinates and plans the API call sequence
3. **API Execution** — Agent calls the Points API, extracts the forecast URL, then calls the Forecast API
4. **AI Processing** — Claude converts raw JSON into a clean, human-friendly summary
5. **Response** — User gets a clear forecast
